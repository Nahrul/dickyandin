...
async function add() {
  ...
  const hadir = document.querySelector('input[name="attendance"]:checked').value;
	...
	try {
        ...
        const url = `https://script.google.com/macros/s/AKfycbxc2LFR6TrIubY5CC-F5Yfhi8PkF5XHmwv5gLi6BBWCiSD1gblpNp97BkQ-xhgfld6D/exec?action=add&table=andin&data=${dataStr}`;
...
function displayGuestbook(data) {
    ...
    data.forEach(item => {
        ...
        let attendClass = '';
        let attendLabel = '';
        if (item.hadir === 'Attend' || item.hadir === 'Ya') {
            attendClass = 'attend-yes';
            attendLabel = 'Attend';
        } else if (item.hadir === 'Mungkin') {
            attendClass = 'attend-maybe';
            attendLabel = 'Maybe';
        } else if (item.hadir === 'Tidak') {
...
async function fetchData() {
    try {
        const response = await fetch('https://script.google.com/macros/s/AKfycbxc2LFR6TrIubY5CC-F5Yfhi8PkF5XHmwv5gLi6BBWCiSD1gblpNp97BkQ-xhgfld6D/exec?table=andin&action=getAll');
